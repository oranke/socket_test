...
DLL 사용자 입장에서는 패킷 전송시 패킷아이디(PID)와 디코딩된 패킷 데이터가 제공된다. 
...
패킷은 TPacketRec 의 PID 로 구분되므로 큰 분류로 255가지가 된다. 
하지만 실제 구현에서는 이것으로 모자라므로, 일반적으로 Data 영역의 첫번째 바이트를 SPID (Sub-PID) 로 정의해 소분류로 사용한다. 
...
이를 통해 이론적으로 65535가지의 패킷종류를 구분지을 수 있으며, 이는 어지간한 중대형 온라인 게임에 충분한 종류이다. 
